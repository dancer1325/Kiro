...
Source: https://kiro.dev/docs/guides/learn-by-playing/00-setup/
...
1Clone the RepositoryClone the open source code repository and switch to the challenge branch:bashgit clone git@github.com:kirodotdev/spirit-of-kiro.git
cd spirit-of-kiro/
...
2Prerequisite DependenciesYou'll need the following dependencies installed:
...
3Verify DependenciesRun the dependency check script to verify everything is set up correctly:sh./scripts/check-dependencies.sh
Loading image...4Deploy Cognito User PoolDeploy an Amazon Cognito user pool for authentication (available in AWS Free Tier):bash./scripts/deploy-cognito.sh game-auth
InfoYou can substitute 'game-auth' with your own custom CloudFormation stack name.Loading image...5Build and LaunchBuild and launch the game stack using either Docker or Podman:bashpodman compose build &&
podman compose up \
--watch \
--remove-orphans \
--timeout 0 \
...
runs should complete in seconds.Loading image...TipIf this is your first time using Podman, you will need to
run podman machine init && podman machine start before using Podman.After completion, you should see the game containers running in your container interface.
The following example shows the Podman UI:
Loading image...You can use Control + C or Command + C to stop the entire stack.InfoWhen using Podman, on some operating systems the virtual machine can experience a
...
the following command:shpodman machine ssh sudo systemctl restart chronyd.service
6Bootstrap databaseWhen your stack launches locally, it also launches DynamoDB local,
...
tables to be created in this DynamoDB container.While the game stack is still running, open a new terminal and use the
following commands to automatically create the necessary tables:shpodman exec server mkdir -p /app/server/iac &&
...
Loading image...InfoThe DynamoDB database is persisted across restarts, by saving it to
...
file, restart the game stack, and rerun this database bootstrap command.7Test it outItâs time to verify that everything is working properly. Try the following command
first to make sure that the game server is running locally:shcurl localhost:8080
You should see the response OKNext open your web browser and put the following address into the address bar:localhost:5173
...
randomized âshopâ where you can indirectly buy from other players,
or buy back things you lost.
Done playing with your copy of the game? Let's get to work on the first task:
Steering Kiro, and improving the game homepagePage updated: November 16, 2025Learn by playingSteering Kiro, and improving the game homepage