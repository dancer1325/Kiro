...
Source: https://kiro.dev/docs/mcp/usage/
...
View all connected servers and their available tools
...
Via the Kiro Panel:
Open the Kiro panel and navigate to MCP servers
Expand a server to see its available tools
Click on a tool to see options:
Enable - Activate a disabled tool
Disable - Temporarily disable a tool without removing the server
Disabled tools show a "Disabled" label and won't be available to Kiro
Via JSON Config:
...
Add tool names to the autoApprove array:
json{
"mcpServers": {
"aws-docs": {
"autoApprove": [
...
Save the file (Cmd+S) to apply changes automatically
...
Use the Ask Kiro feature to resolve errors
Incorrect results
...
For information on available MCP servers and their tools, see the MCP Servers page.Page updated: November 16, 2025Server directoryBest practices