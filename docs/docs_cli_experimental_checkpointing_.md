...
Source: https://kiro.dev/docs/cli/experimental/checkpointing/
...
bash/checkpoint expand <tag>
Example:
> /checkpoint expand 2
[2] 2025-09-18 14:07:10 - add tests
ââ [2.1] write: Add minimal test cases to two_sum.py (modified 1)
/checkpoint diff
Compare checkpoints or compare with current state:
bash/checkpoint diff <tag1> [tag2|HEAD]
Examples:
bash/checkpoint diff 1 2        # Compare checkpoint 1 to checkpoint 2
/checkpoint diff 1 HEAD     # Compare checkpoint 1 to current state
/checkpoint diff 1          # Same as above (HEAD is default)
/checkpoint restore
Restore to a checkpoint:
bash/checkpoint restore [<tag>] [--hard]
...
Examples:
bash/checkpoint restore 2       # Restore to checkpoint 2 (soft)
...
bash/checkpoint restore 2 --hard
What happens:
Modified files are reverted to checkpoint state
Deleted files are restored
...
â ï¸ Warning: Hard restore permanently deletes files. Use with caution.
Use cases
...
ââ [2.1] write: Create auth.py (+1 file)
ââ [2.2] write: Update routes.py (modified 1)
...
Enable at session start: Turn on for complex work
Check progress: Use /checkpoint list periodically
Compare approaches: Use /checkpoint diff to see changes
Restore when needed: Go back to known good states
Clean up: Remove shadow repo when session ends
Limitations
Session scope
...
Important notes
â ï¸ Checkpointing creates temporary git repositories that are cleaned up when the session ends.
â ï¸ Use caution with --hard restore as it permanently deletes files.
â ï¸ Checkpoints are session-scoped and don't persist across sessions.
â ï¸ Conversation history unwinds when restoring to maintain consistency.
Related features
...
Configure settings
Page updated: November 18, 2025Thinking toolDelegate