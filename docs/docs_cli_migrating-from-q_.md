...
Source: https://kiro.dev/docs/cli/migrating-from-q/
...
AreaKiro CLIQ Developer CLIInstallationnative installdmg and zip based installAuthenticationGitHub, Gmail, BuilderId, IAM Identity CenterBuilderId, IAM Identity CenterEntry pointkiro-cliq / q chatRulesKiro steeringAmazon Q rulesSubscriptionsQ Developer and KiroQ Developer and KiroFeaturesMCP, Custom agents, hooksMCP, Custom agents, hooksLicenseAWS Intellectual Property LicenseApache
See the Authentication guide for details.
Frequently asked questions
1. What is Kiro, and how does it relate to Q Developer?
Kiro is a standalone agentic development experience that helps you go from concept to production with spec-driven development. From simple to complex tasks, Kiro works alongside you to turn prompts into detailed specs, then into working code, docs, and testsâso what you build is exactly what you want and ready to share with your team. Kiro leverages some of the advanced Q Developer functionality and technology (agent mode, MCP, steering, CLI) and adds a streamlined but opinionated developer experience for working with AI which feels familiar because it's based on Q Developer CLI.
...
3. What if I donât want to upgrade to the Kiro CLI? Can I keep my Q Developer CLI access?
...
We are making it easy for your team to work with Kiro regardless of whether developers prefer Kiro IDE or Kiro CLI. To learn more about the changes, review the changes in detail.
...
11. Do you provide an output indemnity for Kiro subscribers? If Iâm a Q Developer Pro tier user and I upgrade to a paid Kiro subscription, does the output indemnity apply to me?
...
Linux x86-64
Linux ARM
WarningAny new prompts, agents and steering files you created using Kiro CLI will not be available in Amazon Q Developer CLI. Refer to the configuration file path section to copy files.
Kiro CLI Changes
...
Prompts, agents from ~/.aws/amazonq folder are copied to ~/.kiro folder with the same names
MCP configuration from ~/.aws/amazonq/mcp.json is copied to ~/.kiro/settings/mcp.json. If there are conflicts, skip copying those MCP servers
Files from rules folder in ~/.aws/amazonq folder are copied to ~/.kiro/steering folder with the same names
A cli.json file is created with settings from Amazon Q Developer CLI
Kiro CLI will continue to read .amazonq folder in your project. So your existing rules, agents, and MCP settings in your project will continue to work as-is
...
ImportantWhen you add new project level prompts or agents, they will be saved to the .kiro folder in the project and not .amazonq folder
q and q chat will continue to work, although we recommend using kiro-cli (both kiro-cli and kiro-cli chat will work)
Tools names have been simplified, but existing tool names will continue to work:
...
ConfigurationScopeKiro CLIQ Developer CLIMCP serversUser~/.kiro/settings/mcp.json~/.aws/amazonq/mcp.jsonWorkspace.kiro/settings/mcp.json.amazonq/mcp.jsonPromptsUser~/.kiro/prompts~/.aws/amazonq/promptsWorkspace.kiro/prompts.amazonq/promptsCustom agentsUser~/.kiro/agents~/.aws/amazonq/cli-agentsWorkspace.kiro/agents.amazonq/cli-agentsRules / SteeringUser~/.kiro/steering~/.aws/amazonq/rulesWorkspace.kiro/steering.amazonq/rulesSettingsGlobal~/.kiro/settings/cli.json-
Important changes
...
ImportantIf your project has both .kiro and .amazonq folders, configuration will be loaded from the .kiro folder. You will see a warning about this when you start a new session.
Getting help
If you encounter issues during migration:
Check the CLI Commands Reference
Review Chat documentation
Contact support through the Kiro dashboard
Next steps
Explore Chat features
Learn about Custom Agents
Try MCP integration
Set up Agent Hooks
Page updated: November 19, 2025Settings