...
WarningOnly add MCP servers from trusted sources, and review all applicable server licensing information and documentation. Kiro is not responsible for any third-party MCP servers or other packages.
...
Available tools
Tool NameDescriptionmcp_aws_docs_search_documentationSearch AWS documentation for specific topicsmcp_aws_docs_read_documentationRead AWS documentation pages in markdown formatmcp_aws_docs_recommendGet content recommendations related to a documentation page
Usage examples
...
Setup instructions
WarningThe previously recommended @modelcontextprotocol/server-github package has been archived. GitHub now recommends using their official Docker-based MCP server.
Prerequisites
...
Create a .kiro/settings/mcp.json file in your workspace directory (or edit if it already exists)
...
CategoryTool NameDescriptionRepository Toolssearch_repositoriesSearch for GitHub repositoriesRepository Toolslist_branchesList branches in a repositoryIssue Toolslist_issuesList issues in a repositoryIssue Toolsupdate_issueUpdate an existing issueIssue Toolsadd_issue_commentAdd a comment to an issuePull Request Toolscreate_pull_requestCreate a new pull request
Available toolsets
...
Return to MCP Overview
Page updated: November 16, 2025MCPSecurity