...
bash    /save [path] â Saves your current conversation to a JSON file.
...
/load [path] â Loads a conversation from a previously saved JSON file
Example: /load ./my-project-conversation.json
InfoThe /save and /load commands operate independently of the directory where the conversation was originally created. When loading a conversation, be mindful that it will replace your current conversation regardless of which directory it was saved from.Page updated: November 16, 2025AuthenticationModel selection